...
export default function CompletedLeadsPage() {
  ...
  const sortedProjects = useMemo(() => {
    if (!projects) return [];
    return [...projects].sort((a, b) => b.updatedAt.toDate().getTime() - a.updatedAt.toDate().getTime());
  }, [projects]);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight">Completed Leads</h1>
        <p className="text-muted-foreground">A record of all successfully completed projects.</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>Completed Projects</CardTitle>
          <CardDescription>
            These projects have been marked as 'completed' by the writing team.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
                ...
                {sortedProjects.map((project) => {
                  ...
                  return (
                    <TableRow key={project.id}>
                      ...
                      <TableCell>
                        {project.updatedAt ? format(project.updatedAt.toDate(), 'PPP') : 'N/A'}
                      </TableCell>
                    </TableRow>
                  )
                })}
            ...
            <div className="text-center p-12 text-muted-foreground">
                ...
                <h3 className="text-lg font-semibold">No Completed Leads</h3>
                <p>There are currently no projects with a 'completed' status.</p>
            </div>
          )}
        </CardContent>
      </Card>
